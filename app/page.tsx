...
export default async function Home() {
  ...
  let topBlogs: Array<{ id: string; title: string; slug: string; excerpt: string | null; category: string; featuredImage: string | null; publishedAt: Date | null; createdAt: Date }> = [];
  ...
  try {
    topBlogs = await prisma.blogPost.findMany({
            ...
            {topBlogs.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {topBlogs.map((post) => (
                  <article
                    key={post.id}
                    className="bg-white dark:bg-[#212935] rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all group"
                  >
                    ...
                    <div className="p-6">
                      <h4 className="text-xl font-bold text-slate-900 dark:text-white mb-2 group-hover:text-primary transition-colors leading-tight line-clamp-2">
                        {post.title}
                      </h4>
